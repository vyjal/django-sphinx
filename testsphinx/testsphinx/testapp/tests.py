...
class TestSphinxQuerySet(TestCase):
    ...
    def test_model_queryset_filter(self):
        ...
        self.assertRaises(AttributeError, qs._check_field, non_rel_field, None)
        ...
        self.assertEqual({'related': [2]}, qs._process_filter({}, related=fk_obj))
        # в том числе для range-запросов
        self.assertEqual({'related__in': [2]}, qs._process_filter({}, related__in=fk_obj))
        # но при этом должен выдавать ошибку, если поле не связано с переданным объектом
        self.assertRaises(AttributeError, qs._process_filter, {}, name=fk_obj)
        # а так же если поле отсутствует в модели
        self.assertRaises(FieldDoesNotExist, qs._process_filter, {}, some_field=fk_obj)

        # для QuerySet должен возвращать список первичных ключей из выборки
        self.assertEqual({'related__in': [2,3]}, qs._process_filter({}, related__in=fk_qs))
        # но при этом должен выдавать ошибку, если в QuerySet содержатся объекты не того типа
        self.assertRaises(TypeError, qs._process_filter, {}, related__in=m2m_qs)

        # фильтрация по полям связанных моделей пока не реализована
        self.assertRaises(NotImplementedError, qs._process_filter, {}, related__name__in=fk_qs)
        self.assertRaises(NotImplementedError, qs._process_filter, {}, related__name=fk_obj)
        self.assertRaises(NotImplementedError, qs._process_filter, {}, related__name__exact=fk_obj)